...
class repositories_iq_another(repositories_iq_core,irepositories_iq):

    def __init__(self,cursor):

        super().__init__(cursor)

        self.start_date = config("START_ANOTHER")

        self.end_date = config("END_ANOTHER")

        self.start_date_simple = config("START_ANOTHER")

        self.end_date_simple = config("END_ANOTHER")
        
    def set_complement_start_date(self,date):

        self.start_date = str(date) + self.start_date_simple

        return True

    def set_complement_end_date(self,date):

        self.end_date = str(date) + self.end_date_simple
    ...
    def get_sum_entrys_date(self,date):
        ...
        try:
            ...
            query = "SELECT IFNULL(SUM(samb_entrys_results.result), 0) as result FROM samb_entrys_results WHERE date_format(samb_entrys_results.registration_date, %s) = %s AND samb_entrys_results.registration_date>%s AND samb_entrys_results.registration_date<%s"

            self.cursor_db.execute(query, ('%Y%m%d', date,self.start_date,self.end_date))
    ...
    def get_type_manager_day(self,day):
        ...
        try:
            ...
            query = "SELECT samb_manager_days.type_another AS type, samb_manager_days.money_another AS money, samb_manager_days.profit_another AS profit, samb_manager_days.loss_another AS loss FROM samb_manager_days WHERE samb_manager_days.day_number = %s"
